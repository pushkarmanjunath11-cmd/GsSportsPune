...
export default function AddProductPage() {
  ...
  const [form, setForm] = useState({ name: '', price: '', originalPrice: '', description: '', longDescription: '', moq: '10', customizable: true, featured: false, isNew: false })
  ...
  const sizes = allSizes[category as keyof typeof allSizes] || allSizes.jerseys
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div style={{ background: '#111', border: '1px solid rgba(255,255,255,0.05)', padding: 24, position: 'relative' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div><label style={lbl}>Price (₹)</label><input name="price" type="number" value={form.price} onChange={handle} placeholder="699" style={inp} /></div>
                <div><label style={lbl}>Original Price (₹)</label><input name="originalPrice" type="number" value={form.originalPrice} onChange={handle} placeholder="999" style={inp} /></div>
              </div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {sizes.map((s) => (
                <button key={s} onClick={() => toggleSize(s)}
                  style={{ padding: '8px 14px', fontSize: 12, fontWeight: 700, background: selectedSizes.includes(s) ? '#e8162a' : 'rgba(255,255,255,0.05)', color: selectedSizes.includes(s) ? '#fff' : 'rgba(245,245,245,0.4)', border: selectedSizes.includes(s) ? '1px solid #e8162a' : '1px solid rgba(255,255,255,0.08)', cursor: 'pointer', transition: 'all 0.2s' }}>
                  {s}
                </button>
              ))}
            </div>
          ...
          <div style={{ background: '#111', border: '1px solid rgba(255,255,255,0.05)', padding: 24, position: 'relative' }}>
            ...
            <div style={{ border: '2px dashed rgba(255,255,255,0.08)', padding: 48, textAlign: 'center', cursor: 'pointer' }}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ background: '#111', border: '1px solid rgba(255,255,255,0.05)', padding: 24, position: 'relative' }}>
            ...
            <select value={category} onChange={e => { setCategory(e.target.value); setSelectedSizes([]) }}
