...
export function CartDrawer() {
  ...
  return (
    <>
      ...
      <aside className={`fixed top-0 right-0 h-full w-full max-w-md bg-[#0d0d0d] z-50 flex flex-col border-l border-white/5 transition-transform duration-500 ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>

        <div className="flex items-center justify-between px-6 py-5 border-b border-white/5">
          <h2 className="font-display font-800 text-2xl uppercase text-white">Your Cart
            {items.length > 0 && <span className="text-red-500 ml-2">({items.length})</span>}
          </h2>
          <button onClick={closeCart} className="text-white/40 hover:text-white transition-colors"><X size={20} /></button>
        </div>
                ...
                <li key={item.product.id} className="flex gap-4">
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-start gap-2">
                      ...
                      <button onClick={() => removeItem(item.product.id)} className="text-white/25 hover:text-red-400 transition-colors"><X size={14} /></button>
                    </div>
                    {item.selectedSize && <p className="text-xs text-white/30 mt-1">Size: {item.selectedSize}</p>}
                    <div className="flex items-center justify-between mt-3">
                      <div className="flex items-center border border-white/10">
                        <button onClick={() => updateQuantity(item.product.id, item.quantity - 1)} className="p-1.5 text-white/40 hover:text-white"><Minus size={12} /></button>
                        <span className="px-3 text-sm text-white">{item.quantity}</span>
                        <button onClick={() => updateQuantity(item.product.id, item.quantity + 1)} className="p-1.5 text-white/40 hover:text-white"><Plus size={12} /></button>
                      </div>
